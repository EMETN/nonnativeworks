...
export const POST: APIRoute = async ({ request }) => {
    ...
    const { company_name, keep_countries } = (body ?? {}) as Record<
        string,
        unknown
    >;
    ...
    if (!Array.isArray(keep_countries) || keep_countries.length === 0) {
        return json(
            {
                error: 'keep_countries must be a non-empty array of country slugs',
            },
            400,
        );
    }
    ...
    if (lookupErr) {
        console.error(
            'cleanup-stale-countries lookup error:',
            lookupErr.message,
        );
    ...
    if (keepIdSet.size === 0) {
        ...
        return json(
            { error: 'No matching countries found for keep_countries' },
            400,
        );
    }
    ...
    if (deleteErr) {
        console.error(
            'cleanup-stale-countries delete error:',
            deleteErr.message,
        );
    ...
    console.log(
        `cleanup-stale-countries: deleted ${staleIds.length} stale rows for "${company_name}"`,
    );
