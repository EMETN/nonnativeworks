...
async function scrape(rawUrl: string): Promise<ScrapeResult> {
  ...
  const careerHostname = (() => {
    try {
      return new URL(careerUrl).hostname.toLowerCase().replace(/^www\./, "");
    } catch {
      return "";
    }
  })();
  ...
  if (resolvedAts && detection.companySlug) {
    try {
      if (resolvedAts === "greenhouse") {
        ...
        ({ jobs: rawJobs, companyName } = await fetchAshbyJobsAndCompanyName(
  ...
  return new Promise((resolve, reject) => {
    ...
    py.on("close", (code) => {
      ...
      try {
        ...
        reject(
          new Error(`Python scraper returned invalid JSON. ${stderr.trim()}`),
        );
      }
    });
