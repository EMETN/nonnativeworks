...
export default function Scraper() {
            ...
            setPhase({
                ...
                data: {
                    ...
                    skipped_unknown_location:
                        data.skipped_unknown_location ?? 0,
    ...
    function setReviewData(updater: (prev: ReviewData) => ReviewData) {
        setPhase((prev) =>
            prev.kind === 'review'
                ? { kind: 'review', data: updater(prev.data) }
                : prev,
        );
    }
                            ...
                            <ul class="space-y-0.5 text-xs">
                                {phase.uploadResult.results.map((r, i) => (
                                    <li key={i}>
                                        ...
                                        · {r.country} · {r.positions} positions
                                    </li>
                                ))}
                            </ul>
                        </div>
    ...
    return (
        <div class="space-y-5">
            ...
            <div class="flex flex-wrap items-center gap-2 text-sm">
                ...
                {hasSkipped && (
                    <span class="bg-gray-100 text-gray-600 rounded-full px-2.5 py-0.5 text-xs font-medium">
                        {data.skipped_unknown_location} skipped (unknown
                        location)
                    </span>
                )}
                {hasUntracked && (
                    <span class="bg-gray-100 text-gray-600 rounded-full px-2.5 py-0.5 text-xs font-medium">
                        {data.skipped_untracked_country} skipped (untracked
                        country)
                    </span>
                )}
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                    ...
                    <input
                        ...
                        onInput={(e) =>
                            onCompanyName((e.target as HTMLInputElement).value)
                        }
            ...
            <div class="flex gap-3 pt-1">
                ...
                >
                    Upload {total} {total === 1 ? 'position' : 'positions'} to
                    Supabase
                </button>
                <button
                    onClick={onReset}
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 bg-white">
                            <th class="text-left px-4 py-2 font-medium text-gray-500 w-1/2">
                                Title
                            </th>
        ...
        <tr class="border-b border-gray-50 last:border-0 bg-white">
            ...
            <td class="px-4 py-2">
                <select
                    value={job.category}
                    onChange={(e) =>
                        onCategory((e.target as HTMLSelectElement).value)
                    }
            ...
            <td class="px-4 py-2">
                <div class="flex items-center gap-3">
                    {(
                        ['required', 'advantage', 'neither'] as LanguageStatus[]
                    ).map((status) => (
                        <label
                            key={status}
