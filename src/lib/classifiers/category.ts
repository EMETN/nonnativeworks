...
const CATEGORY_KEYWORD_PATTERNS: Record<
    string,
    Array<{ pattern: RegExp; keyword: string }>
> = {};
...
export function classifyCategoryVerbose(
    ...
    if (jobFunction) {
        for (const [category, patterns] of Object.entries(
            ...
            if (
                score > bestScore ||
                (score === bestScore && earliestPos < bestMatchPos)
            ) {
                bestScore = score;
