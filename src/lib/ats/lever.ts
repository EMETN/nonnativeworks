...
export async function fetchLeverJobs(slug: string, opts?: { eu?: boolean }): Promise<RawJob[]> {
  ...
  return data.map((posting) => {
    ...
    if (posting.lists?.length) {
      ...
      const listsPlain = listsHtml.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
      html = html + '\n' + listsHtml;
      plain = plain + '\n' + listsPlain;
    }
    return {
      ...
      ...(posting.country && { country_code: posting.country.toUpperCase() }),
      ...(posting.workplaceType && { work_model: mapWorkplaceType(posting.workplaceType) }),
    };
  });
}

function mapWorkplaceType(wt: string): 'remote' | 'hybrid' | 'on-site' | undefined {
