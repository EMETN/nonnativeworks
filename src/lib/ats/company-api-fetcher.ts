...
function mapItem(
    ...
    return {
        ...
        country_code: fields.country
            ? (getString(item, fields.country) ?? undefined)
            : undefined,
        url: url || undefined,
...
function expandJobToSecondaryLocations(
    ...
    if (expand.parallelCitiesPath) {
        ...
        const countries = secondaries.filter(
    ...
    for (const sec of secondaries) {
        ...
        extras.push({
            ...
            sourceId: primaryJob.sourceId
                ? `${primaryJob.sourceId}-${countryName}`
                : undefined,
        });
    }
    return extras;
...
async function fetchPageHtml(url: string): Promise<string | undefined> {
    try {
        ...
        if (res.status === 404 || res.status === 410) {
            console.warn(
                `[fetchPageHtml] HTTP ${res.status} (gone) for ${url}`,
            );
...
export async function enrichDescriptions(
    ...
    if (locationRegex) {
        console.log(
            `[enrichDescriptions] location extraction: ${locationsSet} set, ${regexMissed} regex misses, ${htmlFailed} fetch failures`,
        );
    }
}
...
async function fetchAllJobsRaw(
    spec: FetchSpec,
    label = 'primary',
): Promise<RawJob[]> {
    const {
    ...
    if (pagination.type === 'none') {
        ...
        console.log(
            `[${label}] fetched ${jobs.length} jobs (single request)`,
            paginationMeta(data),
        );
        ...
        for (let i = 0; i < MAX_PAGES; i++, page++) {
            const pageUrl =
                method === 'GET' ? buildGetUrl(url, param, page) : url;
            const pageBody =
                method === 'POST'
                    ? param.includes('.')
                        ? setNestedParam(body ?? {}, param, page)
                        : { ...body, [param]: page }
                    : undefined;
            console.log(
                `[${label}] fetching page=${page}${method === 'POST' ? ` body.${param}=${page}` : ` url=${pageUrl}`}`,
            );
            const data = await fetchPage(
                pageUrl,
                method,
            ...
            for (const job of mapItems(
                ...
                jobs.push(job);
                uniqueKeys.add(
                    job.sourceId ?? `${job.title}|${job.location ?? ''}`,
                );
            }
            console.log(
                `[${label}] page=${page} → ${items.length} items, mapped ${jobs.length - before}, cumulative=${jobs.length}`,
                paginationMeta(data),
            );
            ...
            if (totalCount !== undefined && uniqueKeys.size >= totalCount) {
                console.log(
                    `[${label}] stopping: ${uniqueKeys.size} unique jobs fetched of ${totalCount}`,
                );
                break;
            }
        ...
        for (let i = 0; i < MAX_PAGES; i++, offset += pageSize) {
            ...
            console.log(
                `[${label}] fetching finder-offset=${offset} url=${pageUrl}`,
            );
            const data = await fetchPage(
                pageUrl,
                method,
            ...
            if (!data) {
                console.warn(
                    `[${label}] empty response at finder-offset=${offset}`,
                    { url: pageUrl },
                );
                break;
            }
            ...
            console.log(
                `[${label}] finder-offset=${offset} → ${items.length} items, mapped ${jobs.length - before}, cumulative=${jobs.length}`,
                paginationMeta(data),
            );

            if (items.length === 0) {
                console.log(
                    `[${label}] stopping: empty page at finder-offset=${offset}`,
                );
                break;
            }
            if (items.length < pageSize) {
                console.log(
                    `[${label}] stopping: partial page (${items.length} < ${pageSize}) at finder-offset=${offset}`,
                );
                break;
            }
        ...
        for (let i = 0; i < MAX_PAGES; i++) {
            ...
            console.log(
                `[${label}] cursor page=${i + 1} → ${items.length} items, mapped ${jobs.length - before}, cumulative=${jobs.length}`,
                paginationMeta(data),
            );

            if (items.length === 0) {
                console.log(
                    `[${label}] stopping: empty page at cursor page=${i + 1}`,
                );
                break;
            }
            ...
            if (!nextUrl) {
                console.log(
                    `[${label}] stopping: no next_page at cursor page=${i + 1}`,
                );
                break;
            }
...
function deduplicateJobs(jobs: RawJob[], label: string): RawJob[] {
    ...
    const result = jobs.filter((job) => {
        const key =
            job.sourceId ?? `title:${job.title}|loc:${job.location ?? ''}`;
        if (seen.has(key)) {
            console.log(
                `[${label}] duplicate removed — sourceId: ${job.sourceId ?? '(none)'}, title: "${job.title}", location: ${job.location ?? '(none)'}`,
            );
            return false;
        }
        seen.add(key);
    ...
    if (removed > 0) {
        console.log(
            `[${label}] removed ${removed} duplicate(s) (${jobs.length} → ${result.length})`,
        );
    }
    return result;
}
...
export async function fetchCompanyApiJobs(
    ...
    if (config.repeatFor) {
        ...
        console.log(
            `[fetchCompanyApiJobs] repeatFor: ${overrides.length} entries → fetching sequentially`,
        );
        const allRaw: RawJob[] = [];
        for (const override of overrides) {
            ...
            if (method === 'GET') {
                ...
                spec = {
                    ...primarySpec,
                    url: `${primarySpec.url}${sep}${params}`,
                };
            } else {
                spec = {
                    ...primarySpec,
                    body: { ...primarySpec.body, ...override },
                };
            }
                    ...
                    for (const job of jobs) {
                        ...
                        if (job.cities && targetCode) {
                            const filtered = job.cities.filter((city) => {
                                const resolved =
                                    lookupCountryFromLocation(city);
                                return (
                                    resolved.length === 0 ||
                                    resolved.some((i) => i.code === targetCode)
                                );
                            });
                            job.cities =
        ...
        console.log(
            `[fetchCompanyApiJobs] starting primary fetch: ${config.url}`,
        );
    ...
    if (config.secondaryUrl && config.fields.id) {
        ...
        console.log(
            `[fetchCompanyApiJobs] starting secondary (English) fetch: ${config.secondaryUrl}`,
        );
        ...
        if (config.fetchDescription) {
            console.log(
                `[fetchCompanyApiJobs] enriching descriptions for ${englishJobs.length} English jobs`,
            );
        ...
        console.log(
            `[fetchCompanyApiJobs] merge — replaced with English: ${matched}, English-only additions: ${englishOnly}, Finnish-only: ${finnishOnly}`,
        );
    }
    ...
    if (config.fetchDescription || config.locationFromHtml) {
        const locationRegex = config.locationFromHtml
            ? new RegExp(config.locationFromHtml)
            : undefined;
        const descriptionRegex = config.descriptionFromHtml
            ? new RegExp(config.descriptionFromHtml, 's')
            : undefined;
        await enrichDescriptions(
            enrichmentTargets,
            locationRegex,
    ...
    if (config.descriptionApiUrl && config.descriptionApiFields?.length) {
        ...
        console.log(
            `[fetchCompanyApiJobs] enriching descriptions via API for ${enrichmentTargets.length} jobs in tracked countries (of ${primaryJobs.length} total)`,
        );
        await enrichDescriptionsFromApi(
            enrichmentTargets,
            config.descriptionApiUrl,
            itemsPath,
            config.descriptionApiFields,
            headers,
            config.descriptionApiLocationField,
            config.descriptionApiJobFunctionField,
            config.descriptionApiWorkModelField,
        );
    }

    if (
        config.descriptionApiUrl &&
        config.descriptionApiUrlFromJobUrl &&
        config.descriptionApiLanguagesPath
    ) {
        const itemsPath = config.descriptionApiItemsPath ?? 'items.0';
        console.log(
            `[fetchCompanyApiJobs] enriching language requirements via API for ${enrichmentTargets.length} jobs in tracked countries (of ${primaryJobs.length} total)`,
        );
        await enrichLanguageRequirementFromApi(
            enrichmentTargets,
            config.descriptionApiUrl,
            config.descriptionApiUrlFromJobUrl,
            itemsPath,
    ...
    console.log(
        `[fetchCompanyApiJobs] final job count: ${deduplicated.length}`,
    );
    return deduplicated;
}
