...
export async function enrichWorkableDescriptions(jobs: RawJob[], account: string): Promise<void> {
  const targets = jobs.filter((j) => j.sourceId && !titleAppearsNonEnglish(j.title));
