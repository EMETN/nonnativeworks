...
export async function fetchWorkableJobs(account: string): Promise<RawJob[]> {
        ...
        return {
            ...
            location:
                job.city && job.country
                    ? `${job.city}, ${job.country}`
                    : (job.country ?? primaryLocation?.country),
        ...
        await Promise.all(
            targets.slice(i, i + DESCRIPTION_BATCH).map(async (job) => {
                try {
                    ...
                    if (!res.ok) {
                        console.warn(
                            `[workable] v2 API returned ${res.status} for job ${job.sourceId}`,
                        );
                        return;
                    }
                    const detail: WorkableJobDetail = await res.json();
                    if (detail.description)
                        job.descriptionHtml = detail.description;
                } catch (err) {
                    console.warn(
                        `[workable] failed to fetch description for job ${job.sourceId}: ${err}`,
                    );
                }
            }),
        );
