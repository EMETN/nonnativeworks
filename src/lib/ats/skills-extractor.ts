...
export async function loadSkills(): Promise<SkillEntry[]> {
    try {
        ...
        if (error) {
            console.warn(
                '[skills-extractor] Could not load skills:',
                error.message,
            );
            return [];
        }
        ...
        console.warn(
            '[skills-extractor] Unexpected error loading skills:',
            err,
        );
        return [];
    }
...
const EDUCATION_PATTERNS: { level: EducationLevel; patterns: RegExp[] }[] = [
    ...
    },
    {
        level: 'vocational',
        patterns: [
            /\bvocational\s+(?:qualification|degree|education|certificate|diploma)\b/i,
            /\bvocational\s+training\s+(?:qualification|program(?:me)?|certificate|degree)\b/i,
            /\bapprenticeship\b/i,
