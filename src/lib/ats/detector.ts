 ...
 */
const HOSTNAME_SLUG_OVERRIDES: Record<
    string,
    { ats: AtsDetectionResult['ats']; slug: string }
> = {
...
export function detectAts(url: string): AtsDetectionResult {
    try {
        ...
        if (
            ...
            if (match && match[1] !== 'embed')
                return { ats: 'greenhouse', companySlug: match[1] };
        }
        ...
        if (hostname === 'jobs.lever.co' || hostname === 'jobs.eu.lever.co') {
            ...
            if (match)
                return {
                    ats: 'lever',
                    companySlug: match[1],
                    leverEu: hostname === 'jobs.eu.lever.co',
                };
        }
