...
function formatLanguageSignal(pos: PositionLogEntry): string {
    ...
    switch (sig.phase) {
        case '1a':
            return `1a: non-ASCII title${m}`;
        case '1b':
            return `1b: advantage phrase${m}`;
        case '1b-chars':
            return `1b-chars: ${sig.description}${m}`;
        case '1c':
            return `1c: tinyld${m}`;
        case '2a':
            return `2a: ${sig.description}${m}`;
        case '2a-nordic':
            return `2a-nordic: ${sig.description}${m}`;
        case '2a-cross':
            return `2a-cross: ${sig.description}${m}`;
        case '2b':
            return `2b: location-conditional${m}`;
        case '2c':
            return `2c: English-only${m}`;
        default:
            return sig.description + m;
    }
}
...
function formatCategory(pos: PositionLogEntry): string {
    ...
    const sourceSuffix =
        pos.categorySource === 'description'
            ? '/desc'
            : pos.categorySource === 'jobFunction'
              ? '/api'
              : '';
...
export function logScrapeRun(params: {
    ...
    lines.push(sep);
    lines.push(
        `SCRAPE  ${timestamp}  ${params.companyName}  (${params.ats ?? 'unknown'})`,
    );
    lines.push(`URL     ${params.careerUrl}`);
    if (
        params.outcomeCacheTotal !== undefined &&
        params.outcomeCacheTotal > 0
    ) {
        const hits = params.outcomeCacheHits ?? 0;
        const misses = params.outcomeCacheTotal - hits;
        lines.push(
            `CACHE   ${hits} hits  ${misses} misses  (${params.outcomeCacheTotal} total)`,
        );
    }
    lines.push(sep);
    lines.push('');
    ...
    const byCountry = new Map<
        string,
        { name: string; positions: PositionLogEntry[] }
    >();
    for (const pos of params.positions) {
        if (!byCountry.has(pos.countryCode)) {
            byCountry.set(pos.countryCode, {
                name: pos.countryName,
                positions: [],
            });
        }
    ...
    for (const [code, { name, positions }] of byCountry) {
        ...
        lines.push(
            `  ${name.toUpperCase()} (${code})  —  ${count} position${count !== 1 ? 's' : ''}`,
        );
        lines.push(`  ${sub}`);
        for (const pos of positions) {
            const icon = pos.requires_native_language
                ? '✗'
            ...
            if (pos.city && pos.city.length > 0)
                locationParts.push(pos.city.join(', '));
            if (locationParts.length > 0) {
                lines.push(`       ${locationParts.join(' · ')}`);
            }
            const metaParts: string[] = [];
            if (pos.required_education)
                metaParts.push(`edu:${pos.required_education}`);
            if (pos.skills && pos.skills.length > 0)
                metaParts.push(pos.skills.join(', '));
    ...
    if (params.skippedUnknownLocation > 0)
        skippedParts.push(`${params.skippedUnknownLocation} unknown location`);
    if (params.skippedUntrackedCountry > 0)
        skippedParts.push(
            `${params.skippedUntrackedCountry} untracked country`,
        );
